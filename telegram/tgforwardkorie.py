...
def ielambda(s):
  from re import sub
  return sub(r'([^(, ]+)[ ]?[=][>][ ]?([^),]+)', r'function(\1){\2;}', s)

#from tgforward import forward
#forward_loop(ielambda())
...
if __name__ == '__main__':
  ...
  while True:
    ...
    while True:
      ...
      drv.execute_script(ielambda("mm.getSearch(%d, '%s').then(x => window.o=x, e => window.o=false)") % (pid, msg))
      mid = waitrst(drv)
